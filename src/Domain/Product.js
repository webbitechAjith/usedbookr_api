...
function Product() {
  ...
  const { isLiked, isAdded, allbookDetails, userIdShop, userIdLike, filterBookCategory, likedProducts, likescount, shopProducts, filterCategory, shopcount, minPrice, priceFilter, filteredProducts, productIdDetails, searchfield } = useSelector((state) => state.usedbookr_product)
                  ...
                  {filterCategory?.length > 0 || filterBookCategory.length > 0 ?
                    ...
                    <>
                      {allbookDetails?.length > 0 ?
                        <>
                          {allbookDetails && allbookDetails.slice((currentPage - 1) * productsPerPage, currentPage * productsPerPage).map((book, index) => {
                            return (
                              <>
                ...
                <div className='row m-0  py-4 bestseller'>
                  {filterCategory?.length > 0 ?
                    <>
                      ...
                      <div className='row m-0 gy-2 total-books mt-3'>
                        <div className='col-lg-6 col-12'>
                          <p className='text-center'>Total Books - {allbookDetails?.length}</p>
                        </div>
                        <div className='col-lg-6 col-12'>
